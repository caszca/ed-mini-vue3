...
```
tranform就是在ast抽象语法树的基础上，进行一些增删改查等操作。那么为了能够更好更具有扩展性的
操作，让用户自己操作，选择让用户将操作封装成函数，系统内部调用传递给用户关键值，然后用户就可以
利用关键值进行特殊操作，与webpack中的plugin一样的思想。
```

### 插槽语法时

```
针对插槽语法时，需要引入一些函数，而普通的text不需要引入。那么针对一个ast树，transform时
到底添不添加这个引入句子，我们可以在transform中的遍历时，查看有没有遇到插槽节点，有的话在ast
根节点加上一个数组，数组存储插槽语法引入时所需的函数名。

针对生成render函数里return后的字符串，可能会遇到额外的参数，例如_ctx，这时一般是用
transform插件完成。
```

### Element编译时

针对<div>hi,{{message}}</div>，编译后如下
...
```
目前针对element的children时，当text与插槽语法相连时，中间需要一个+，
做法是将text与插槽节点合成一个符合节点，如下图，在transform步骤中做，然后children节点又
交给genNode处理
```
